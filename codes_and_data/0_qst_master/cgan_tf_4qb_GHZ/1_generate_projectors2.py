...
# Local paths
local_path = "0_qst_master/cgan_tf_4qb_GHZ/%s"
data_path = "0_qst_master/cgan_tf_4qb_GHZ/data/%s"

# Number of qubits:
n = 4

# Load state
estado = np.loadtxt(data_path % "rho_4qubits_GHZ.txt")
#estado = np.loadtxt(local_path % "rho_4qubits_GHZ_dep_0.3588068394402844.txt")

rho = Qobj(estado[:])

# Projectors for 1-qubit
...
ax.set_title('Expectation values for every measurement settings of simulation cGAN-2a - No addition of noise')
plt.legend(fontsize='20')
plt.xticks(fontsize='15')
plt.yticks(fontsize='15')
plt.xlabel("Measurement setting", fontsize='20')
